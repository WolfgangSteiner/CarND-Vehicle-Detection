...
class CV2Grid(object):
    ...
    def draw_frame(self, pos, size, fill_color=cvcolor.white, border_color=cvcolor.black, x_anchor="left", y_anchor="top"):
        offset = np.zeros(2,np.int)
    ...
    def text_frame(self, pos, size, text, text_color=cvcolor.black, fill_color=cvcolor.white, border_color=cvcolor.black, x_anchor="left", y_anchor="top", scale=1.0, margin=[20,20]):
        size = np.array(size)
        if size[0] == -1:
